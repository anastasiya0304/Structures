...
3. Для сравнения воспользуемся средствами встроенных библиотек, рассчитаем время, затраченное на операции, покажем сожность операций каждого метода в худшем, среднем и лучшем случае, вычислим затраты по памяти, на основе всех данных покажем достоинства и недостатки каждой структуры.

# Входные и выходные данные

На стандартном потоке ввода задается последовательность команд, где каждая строка содержит ровно одну команду command(Has, Remove, Add, Print, max, min) и значение. Command [key]
key- ключ, целое число
На выходе программы получаем реальную оценку времени, затраченного на выполнение команд и объем используемой памяти.

# Инструкция по запуску тестируемой программы

Необходимо сохранить все прикрепленные файлы, поместить в папку с программой файлы
В командной строке указываем путь к exe файлу, затем указываются два файла 

# Описание тестов

В программе реализуются 2 типа тестов. Проверка корректности работы алгоритмов и тест на проверку скорости выполнения алгоритмов.
Тестовые файлы содержат данные для проверки корректности алгоритмов.
Значения подбирались случайным образом.
Тестовый файл содержит данные, для подсчета времени, затраченного на вставку большого количества данных в структуру.
Тестовый файл содержит данные, для подсчета времени, затраченного на поиск большого количества данных в структуре.
Тестовый файл содержит данные, для подсчета времени, затраченного на удаление большого количества данных в структуре.

# Обработка результатов

Так как программа прошла все тесты и оказалась корректной, проведем тестирование для подсчета времени.
Первым рассмотрен случай, где на вход подавалось комбинация из различных видов операций (значения выбирались случайным образом). На выходе мы получили разницу во времени в . В среднем оба алгоритма должны работать за приблизительно равное время, следовательно результаты можно принять равными.
Далее, рассмотрим файл, содержащих больше операций добавления. Теоретически, в списке с пропусками операция добавления должна производиться быстрее. На практике получена разница в , из чего следует, что список с пропусками для данной операции работает быстрее.
Далее, рассмотрим файл, содержащий больше операций удаления. Теоретически, в списке с пропусками операция удаления должна производиться быстрее. На практике получена разница в , из чего следует, что список с пропусками для данной операции работает быстрее.
Далее, рассмотрим файл, содержащий больше операций поиска.
Теоретически, поиск в АВЛ-деревьях операция поиска должна производиться быстрее. На практике получена разница в , из чего следует, что АВЛ-деревья для данной операции работают быстрее.
Вывод: списки с пропусками — это простая структура данных, которая может использоваться вместо сбалансированных деревьев в большинстве случаев. Алгоритмы очень легко реализовывать, расширять и изменять. Обычные реализации операций на списках с пропусками примерно такие же быстрые, как высокооптимизированные реализации на сбалансированных деревьях и в значительной степени быстрее обычных, невысокооптимизированных реализаций. Так как, в большенстве случаев списки с пропусками справляются с командами быстрее, на практике выгоднее использовать именно списки с пропусками.
